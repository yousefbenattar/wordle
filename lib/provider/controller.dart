...
class Controller extends ChangeNotifier {
  ...
  setKeyTapped({required String value}) {
    if (value == "ENTER") {
      ...
      if (currentTile < 5 * (currentrow + 1)) {
        tilesEntered.add(
            TileModel(letter: value, answerStages: AnswerStages.notAnswered));
        currentTile++;
      }
    }
    debugPrint('current Tile $currentTile / current row $currentrow');
    notifyListeners();
  }
    ...
    if (guessedWord == correctWord) {
      ...
      for (int i = 0; i < 5; i++) {
        if (guessedWord[i] == correctWord[i]) {
          ...
          tilesEntered[i + (currentrow * 5)].answerStages =
